...
public class AStarState
{
    ...
    public Waypoint getMinOpenWaypoint()
    {
        Waypoint point = null;
        double min = 10000;
        double resultWaypoint = 0;
        for (Waypoint i : openWaypoints.values()){
            resultWaypoint = i.getTotalCost();
            if(resultWaypoint < min){
                min = resultWaypoint;
                point = i;
            }
        }
    ...
    {
        Waypoint shortPoint = openWaypoints.get(newWP.loc);
        if(shortPoint == null || newWP.getPreviousCost() < shortPoint.getPreviousCost()){
    ...
    {
        Waypoint delPoint = openWaypoints.remove(loc);
        closedWaypoints.put(loc, delPoint);
    }
